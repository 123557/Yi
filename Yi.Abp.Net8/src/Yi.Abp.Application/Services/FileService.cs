...
namespace Yi.Abp.Application.Services
{
    public class FileService : ApplicationService, IFileService
    {
        ...
        {
            var path = await GetFileReturnPathAsync(code, false);
        ...
        public async Task<string> GetFileReturnPathAsync(Guid code, bool? isThumbnail)
        {
        ...
        [Route("filelist")]
        public async Task<List<FileGetListOutputDto>> Post([FromForm] IFormFileCollection file, [FromForm] Guid? questionid)
        {
            if (file.Count() == 0)
            {
                throw new ArgumentException("文件上传为空！");
            ...
            foreach (var f in file)
            {
                ...
                if (questionid != null)
                {
                    data.QuestionId = (Guid)questionid;
                }
                //生成文件
                using (var stream = new FileStream(filePath, FileMode.CreateNew, FileAccess.ReadWrite))
                {
        ...
        [Route("getPicturepath")]
        [HttpGet]
        public async Task<List<FileGetListOutputDto>> getList(Guid questionid)
        {
            
            var entities = await _repository.GetListAsync(it => it.QuestionId == questionid);
            if (entities.Count>0)
            {
